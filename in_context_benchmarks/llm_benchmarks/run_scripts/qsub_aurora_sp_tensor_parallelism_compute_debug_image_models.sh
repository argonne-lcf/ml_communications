...
## SEQ=4096, HID=16384, N_LAYERS=126, Llama 405B - SP=6,12 doesn't work, Trying 4608
...
## RUN ID for PPN=2
#RUN_ID=aurora_tensor_parallel_CB08_ZE02_SEQ${SEQ}_HID${HID}_TP${TP_DEGREE}_SP_L${N_LAYERS}_TL${TIMING_LOOPS}_${PRECISION}_${IN_TYPE}_ELEM_${BUCKET}_N${NNODES}_R${NRANKS_PER_NODE}_T${TRIAL}_$(date +"%Y-%m-%d_%H-%M-%S")

## RUN ID for PPN=4
#RUN_ID=aurora_tensor_parallel_CB081624_ZE0123_SEQ${SEQ}_HID${HID}_TP${TP_DEGREE}_SP_L${N_LAYERS}_TL${TIMING_LOOPS}_${PRECISION}_${IN_TYPE}_ELEM_${BUCKET}_N${NNODES}_R${NRANKS_PER_NODE}_T${TRIAL}_$(date +"%Y-%m-%d_%H-%M-%S")

## RUN ID for PPN=6, Asymmetric GPU placement
#RUN_ID=aurora_tensor_parallel_CB0816245260_ZE012367_SEQ${SEQ}_HID${HID}_TP${TP_DEGREE}_SP_L${N_LAYERS}_TL${TIMING_LOOPS}_${PRECISION}_${IN_TYPE}_ELEM_${BUCKET}_N${NNODES}_R${NRANKS_PER_NODE}_T${TRIAL}_$(date +"%Y-%m-%d_%H-%M-%S")

## RUN ID for PPN=8, NIC balanced case
#RUN_ID=aurora_tensor_parallel_CB08162452606876_ZE01236789_SEQ${SEQ}_HID${HID}_TP${TP_DEGREE}_SP_L${N_LAYERS}_TL${TIMING_LOOPS}_${PRECISION}_${IN_TYPE}_ELEM_${BUCKET}_N${NNODES}_R${NRANKS_PER_NODE}_T${TRIAL}_$(date +"%Y-%m-%d_%H-%M-%S")

## RUN ID for R12 setup, Regular, NIC imbalanced case
RUN_ID=aurora_tensor_parallel_SEQ${SEQ}_HID${HID}_TP${TP_DEGREE}_SP_L${N_LAYERS}_TL${TIMING_LOOPS}_${PRECISION}_${IN_TYPE}_ELEM_${BUCKET}_N${NNODES}_R${NRANKS_PER_NODE}_T${TRIAL}_$(date +"%Y-%m-%d_%H-%M-%S")
...
-bucket ${BUCKET} -sp_switch \
--logging --log_directory=${WORK_DIR}/run_scripts/outdir_aurora/logs/model_sweep_for_sc25/sp_sweep_S16896_1T --log_file=${RUN_ID}.log
