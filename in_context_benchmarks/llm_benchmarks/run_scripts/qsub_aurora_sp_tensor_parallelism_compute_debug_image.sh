...
## Symmetric division, tile 0s of GPUs used only, Performs poorly
...
## RUN ID for PPN=2
#RUN_ID=aurora_tensor_parallel_CB08_ZE02_TP${TP_DEGREE}_SP_L${N_LAYERS}_TL${TIMING_LOOPS}_${PRECISION}_${IN_TYPE}_ELEM_${BUCKET}_N${NNODES}_R${NRANKS_PER_NODE}_T${TRIAL}_$(date +"%Y-%m-%d_%H-%M-%S")

## RUN ID for PPN=4
#RUN_ID=aurora_tensor_parallel_CB081624_ZE0123_TP${TP_DEGREE}_SP_L${N_LAYERS}_TL${TIMING_LOOPS}_${PRECISION}_${IN_TYPE}_ELEM_${BUCKET}_N${NNODES}_R${NRANKS_PER_NODE}_T${TRIAL}_$(date +"%Y-%m-%d_%H-%M-%S")
...
RUN_ID=aurora_tensor_parallel_TP${TP_DEGREE}_SP_L${N_LAYERS}_TL${TIMING_LOOPS}_${PRECISION}_${IN_TYPE}_ELEM_${BUCKET}_N${NNODES}_R${NRANKS_PER_NODE}_T${TRIAL}_$(date +"%Y-%m-%d_%H-%M-%S")
...
--logging --log_directory=${WORK_DIR}/run_scripts/outdir_aurora/logs/sp_sweep_for_sc25 --log_file=${RUN_ID}.log
