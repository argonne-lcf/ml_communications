...
UNITRACE_WRAPPER=${WORK_DIR}/run_scripts/unitrace_wrapper_gt4.sh
...
RUN_ID=sunspot_unitrace_tensor_parallel_TP${TP_DEGREE}_NO_SP_LAYERS${N_LAYERS}_NOWARMUPS_TIMING_LOOPS${TIMING_LOOPS}_${PRECISION}_N${NNODES}_R${NRANKS_PER_NODE}_T${TRIAL}_$(date +"%Y-%m-%d_%H-%M-%S")
...
${UNITRACE_WRAPPER} ${LOG_WRAPPER} python ${WORK_DIR}/tensor_parallel_with_gradient_synchronization.py -dvc "xpu" \
