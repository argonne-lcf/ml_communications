...
parser.add_argument("--dp_only", help="Execute just the DP grad sync step, no TP or SP", action='store_true')
...
def tensor_parallel(N_timing_loop, n_layers, n_iter_grad_sync, 
                    ...
                    mat_h_4h, mat_4h_h, allreduce_grad, warmup=False, dp_only=False):
    ...
    for m in range(N_timing_loop):
        ...
        if dp_only:
            interim2 = partial_interim2 ## No physical significance, dummy
            interim4 = partial_interim4
            pass
        else:
            if args.barrier:
...
if args.trace is not None:
    ...
    with profile(activities=activities, record_shapes=True) as prof:
        result = tensor_parallel(args.iterations, args.number_of_transformer_layers, n_iter_grad_sync, 
                                 ...
                                 mat_h_4h=mat_h_4h, 
                                 mat_4h_h=mat_4h_h, allreduce_grad=allreduce_grad, warmup=False, dp_only=False)
    ...
    if args.dp_only:
        result = tensor_parallel(args.iterations, args.number_of_transformer_layers, n_iter_grad_sync, 
                                 input=input, partial_input=partial_input, partial_interim2=partial_interim2, 
                                 partial_interim4=partial_interim4, attn_W_QKV=attn_W_QKV, attn_WO=attn_WO,
                                 mat_h_4h=mat_h_4h, mat_4h_h=mat_4h_h, allreduce_grad=allreduce_grad, warmup=False, dp_only=True)
    else: 
        result = tensor_parallel(args.iterations, args.number_of_transformer_layers, n_iter_grad_sync, 
                                 input=input, partial_input=partial_input, partial_interim2=partial_interim2, 
                                 partial_interim4=partial_interim4, attn_W_QKV=attn_W_QKV, attn_WO=attn_WO,
                                 mat_h_4h=mat_h_4h, mat_4h_h=mat_4h_h, allreduce_grad=allreduce_grad, warmup=False, dp_only=False)
...
logging.info(f"Grad Sync all individual timings = {T_grad_sync_individual / 1e6} ms")
